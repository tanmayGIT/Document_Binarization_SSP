function [L_p_ForeGround_Image, S_p_Padded] = PerformSymmetryOperation_4(S_p_Padded, thetaGradImage_Padded, NormImag_Padded, ...
    fSureConfusedTextPixelsPadded, strongImagePixels,checkingVal, winX, winY, strokeWidth,fSureTextBinFeaturePixelsPadded)

L_p_ForeGround_Image = strongImagePixels;
...
%                             sumNeighBorThreshCnt = sumNeighBorThreshCnt - 1;
%                         end
                        % counting by twice stroke width
                        neighCnt = neighCnt +1;
                    end
                    
                    if( (strongImagePixels(ii+k, jj+j) == 1) )  % get strong pixel count
                        strongPixelCnt = strongPixelCnt + 1;
                    end
                end
            end
        end
        
        if(strongPixelCnt < (alphaStrokeWidth * strokeWidth)) % not much strong pixel exist at the neighborhood
        ...
        if(validFlag == 1)
            goodQualifyPxCnt = goodQualifyPxCnt +1;
        end
    end
    ...
    if(goodQualifyPxCnt >=checkLimitComp)
        for nSz = 1:1:size(HoldConnecCom{AccesEachCell,1},1)
            ii = HoldConnecCom{AccesEachCell,1}(nSz,1);
            jj = HoldConnecCom{AccesEachCell,1}(nSz,2);
